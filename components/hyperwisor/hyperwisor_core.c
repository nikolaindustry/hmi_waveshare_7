 ...
 * @brief Hyperwisor IoT core orchestrator (S3 port: no GPIO, no OTA)
 */
...
esp_err_t hyperwisor_init(void)
{
    ...
    /* Init subsystems (no GPIO, no OTA on S3 port) */
    ESP_ERROR_CHECK(hyperwisor_wifi_init());
    ESP_ERROR_CHECK(hyperwisor_ws_init());
